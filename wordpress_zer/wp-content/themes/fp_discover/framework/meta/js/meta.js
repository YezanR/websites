	...
	$('.meta-field').on('click','.upload-post-img-btn',function(e){
		...
		custom_uploader.on('select', function() {
			attachment = custom_uploader.state().get('selection').first().toJSON();
				...
				if (thumbnail_url != ''){
					$('.selected-images .image-list ul').append( '<li><div class="thumb"><img src="'+thumbnail_url+'" /><input type="hidden" name="fp_meta_gallery_img_id[]" value="'+thumbnail_id+'" /></div><div class="image-settings"><span class="remove"><i class="icon-remove"></i></span></div></li>' )
				}
		});
